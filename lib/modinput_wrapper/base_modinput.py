...
from sfdc_consts import LOG_FORMAT
...
class BaseModInput(smi.Script):
    ...
    def __init__(self, app_namespace, input_name, use_single_instance=False):
        ...
        self.logger = logging.getLogger(app_namespace + "_" + input_name)
        splunk_home = os.path.abspath(
            os.path.join(os.getcwd(), os.environ.get("SPLUNK_HOME", ""))
        )
        file_path = os.path.join(
            splunk_home, "var", "log", "splunk", f"{app_namespace}_{input_name}.log"
        )
    ...
    def stream_events(self, inputs, ew):
        ...
        try:
            account = list(input_definition.inputs.values())[0].get("account")
            inputStanzaKey = list(input_definition.inputs.keys())[0].split("//")[1]
            cfm = conf_manager.ConfManager(
                input_definition.metadata["session_key"],
            ...
            if (account is not None) and (account not in account_configs):
                self.log_error(
                    "Specified account '{}' linked to input '{}' is not configured. "
                    "You can either configure account in splunk_ta_salesforce_account.conf"
                    " file or on Configurations page of add-on.".format(
                        account, inputStanzaKey
                    )
                )
                sys.exit(0)
    ...
    def load_inputs(self, global_config):
        """Functionality of this method is same as "global_config.inputs.load" method,
           we have made it custom because "global_config.inputs.load" method is raising
           GlobalConfigError whenever one of the input is using account which is deleted.

        :param global_config: object of GlobalConfig
        :return: `dict` of inputs
        """
        if not global_config.inputs._references:
            global_config.inputs._references = global_config.configs.load()
        inputs = {}
        for input_item in global_config.inputs.internal_schema:
            if self.input_type is None or input_item["name"] == self.input_type:
                input_entities = global_config.inputs._load_endpoint(
                    input_item["name"], input_item["entity"]
                )
                # filter unused fields in response
                for input_entity in input_entities:
                    global_config.inputs._filter_fields(input_entity)
                # expand referenced entity
                self._reference(
                    input_entities, input_item, global_config.inputs._references
                )
                inputs[input_item["name"]] = input_entities
        return inputs

    def _reference(self, input_entities, input_item, configs):
        """Functionality of this method is same as "global_config.inputs._reference" method,
           we have made it custom to use it in "load_inputs" method.

        :param input_entities: contain input entities
        :param input_item: contain input item
    ...
    def _input_reference(self, input_type, input_entity, input_schema, configs):
        """Functionality of this method is same as "global_config.inputs._input_reference" method,
           we have made it custom to use it in "_reference" method.

        :param input_type: contain input type
        :param input_entity: contain input entity
        :param input_schema: contain input schema
        :param configs: `dict` of configs
        :return: None
        """
    ...
    def _parse_input_args_from_env(self, inputs):
        ...
        :param inputs:
        """
        data_inputs_options = json.loads(  # noqa: F841 - it is template AOB code
            os.environ.get(DATA_INPUTS_OPTIONS, "[]")
        )
        account_fields = self.get_account_fields()
        checkbox_fields = self.get_checkbox_fields()
        self.input_stanzas = {}
    ...
    def get_proxy(self):
        ...
        Proxy settings include fields "proxy_url", "proxy_port", "proxy_username",
            "proxy_password", "proxy_type" and "proxy_rdns".

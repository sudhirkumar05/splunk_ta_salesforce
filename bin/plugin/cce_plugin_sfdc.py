...
from cloudconnectlib.common import log as cc_log
from cloudconnectlib.core.ext import regex_search, splunk_xml, std_output
from cloudconnectlib.core.plugin import cce_pipeline_plugin
from solnlib import conf_manager
from solnlib.modular_input.event import XMLEvent

logger = cc_log.get_cc_logger()
...
@cce_pipeline_plugin
def get_csv_limit(session_key, app_name):
    """Fetch the csv field size limit from the splunk_ta_salesforce_settings conf file."""
    ...
    try:
        csv_limit = int(conf.get("general").get("csv_limit", DEFAULT_CSV_LIMIT))
...
def read_event_log_file(
    ...
    if "\x00" in stream:
        ...
        stream = stream.replace("\x00", "")

    # Breaks down stream into line separated events
    stream_events = stream.splitlines()

    csv_reader = csv.DictReader(stream_events)
    records = []
    ...
    csv_limit = get_csv_limit(session_key, app_name)
    ...
    for row in csv_reader:
        ...
        event.append('SplunkRetrievedServer="' + server_url + '"')
    ...
    if len(records) > 0:
        ...
        del records
    logger.info(
        "Read %s events collected from csv file id=%s", str(total_records), log_id
    )
    return
...
def build_query(
    ...
    if _to_bool(is_greater_than):
        soql += ">{} ORDER BY {} {} LIMIT {}".format(
            start_date, order_by, sorting_order, limit
        )
    else:
...
def exclude_fields(records, hourly):
    if not hourly:
        ...
        return [
            {k: r[k] for k in (LOG_ID, LOG_EVENT_TYPE, LOG_DATE, CREATED_DATE)}
            for r in records
        ]
...
def refresh_start_date(
    ...
    logger.debug(
        'comparator="%s" previous_empty=%s start_date=%s next_start_date=%s'
        " new_start_date=%s",
        comparator,
        previous_empty,
        start_date,
...
def refresh_checkpoint(name, records, ckpt_mgr):
    ...
    if not records:
        logger.info(
            "No records found in checkpoint, don't need to clean up checkpoint."
        )
        return left_records
    for record in records:
    ...
    logger.info(
        "%s records found in checkpoint. %s files are not finished.",
        num_total,
        num_left,
    )

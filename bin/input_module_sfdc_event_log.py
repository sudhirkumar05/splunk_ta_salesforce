...
from cloudconnectlib.splunktacollectorlib.data_collection.ta_checkpoint_manager import (  # isort: skip
    TACheckPointMgr,
)
...
def _check_access_token(task_config, meta_config, logger, helper_proxy):
    header = {
    ...
    request = {
        "url": "{{server_url}}/services/data/{{API_VERSION}}/sobjects/EventLogFile",
        "method": "GET",
        "headers": header,
    }
    sslconfig = get_sslconfig(meta_config["session_key"])
    task = CCEHTTPRequestTask(
        request, "check_access_token", meta_config, task_config, verify=sslconfig
    )
    task.set_iteration_count(1)
    # Get account name to append in source
    task.add_preprocess_handler(
        "set_var", [task_config.get("account").get("name")], "account_name"
    )
    # Get input name to append in source
    task.add_preprocess_handler(
        "set_var", [task_config.get("stanza_name")], "input_name"
    )
    check_refresh_required = (
        # if_session_expired = True means access token expired
        (
            "check_rest_response",
            ["{{__response__.status_code}}", "{{__response__.body}}"],
            "if_session_expired",
        ),
        # if_session_expired = True means access token refreshed
        (
            "refresh_access_token",
            ["{{if_session_expired}}", task_config, meta_config, logger, helper_proxy],
            "",
        ),
    )
...
def _index_event_log_file(task_config, meta_config):
    ...
    sslconfig = get_sslconfig(meta_config["session_key"])
    task = CCEHTTPRequestTask(
        request, "EventLogFile", meta_config, task_config, verify=sslconfig
    )
    task.set_iteration_count(1)
    task.configure_checkpoint(name="{{event_log_file.Id}}", content={"finished": True})
    process_params = (
        (
            "set_var",
            ["sfdc_event_log://EventLog_{{account_name}}_{{input_name}}"],
            "source",
        ),
        (
            "read_event_log_file",
...
def _build_query(task_config, logger):
    ...
    if lv == "hourly":
        terms = [
            "SELECT Id,EventType,LogDate,CreatedDate FROM EventLogFile WHERE"
            " CreatedDate{{comparator}}{{start_date}}"
        ]
        logger.info(
            "Add-on will only collect event logs which interval is '%s'", interval
        )
        terms.append("AND Interval='%s'" % lv.capitalize())
        terms.append("ORDER BY CreatedDate LIMIT {{limit}}")
    else:
        terms = [
            "SELECT Id,EventType,LogDate FROM EventLogFile WHERE"
            " LogDate{{comparator}}{{start_date}}"
        ]
        if lv == "daily":
            logger.info(
                "Add-on will only collect event logs with an interval of '%s'", interval
            )
            terms.append("AND Interval='%s'" % lv.capitalize())
...
def add_process_pipeline(
    task, task_config, meta_config, logger, helper_proxy, is_retry
):
    ...
    pre_params = (
        ...
        (
            "refresh_checkpoint",
            ["{{name}}", "{{records}}", task._checkpointer],
            "records",
        ),
    ...
    if (
        not is_retry
        and task_config.get("account").get("auth_type")
    ...
    post_params = (
        ...
        (
            "filter_not_indexed_records",
            ["{{records}}", "{{records_on_start_date}}"],
            "records",
        ),
    ...
    if (
        task_config.get("account").get("auth_type")
        and task_config.get("account").get("auth_type") == _OAUTHFLOW
    ):
        task.add_postprocess_handler(
            "get_account_id", [task_config, meta_config, logger, helper_proxy], ""
        )
...
def _list_event_log(
    ...
    if session_active:
        ...
        if (not task_config.get("account").get("auth_type")) or task_config.get(
            "account"
        ).get("auth_type") != _OAUTHFLOW:
            common.check_login_result(task)
        add_process_pipeline(
            task, task_config, meta_config, logger, helper_proxy, is_retry
        )
        return task
...
def _split_records_to_file():
    ...
    task.configure_split(
        method="split_by", source="{{records}}", output="event_log_file"
    )
    return task
...
def collect_events(helper, ew):
    ...
    helper.logger.debug("Started collecting events for {}.".format(task_config["name"]))
    if not common.check_common_parameters(task_config, logger):
        rest.simpleRequest(
            ...
            postargs={
                ...
                "value": (
                    f"Some account configurations are incomplete for input \"{task_config.get('name')}\" in "
                    "Splunk Add-on for Salesforce. "
                    "Go to the Salesforce Add-on [[/app/Splunk_TA_salesforce/configuration|Configuration page]] to "
                    "fix them in order to resume data collection."
                ),
            },
            method="POST",
        )
    ...
    task_config.update({"appname": helper.get_app_name(), "stanza_name": stanza_name})
    ...
    start_date_config_fixed = common.fix_start_date(
        start_date_config, _DEFAULT_START_DATE, logger
    )

    # Check interval field provided by user
    common._check_interval(
        task_config.get("interval", "0"), task_config["name"], logger
    )
    ...
    if ckpt_data:
        ...
        if (
            ckpt_start_date_config is not None
            and common.is_start_date_changed(start_date_config, ckpt_start_date_config)
            and (
            ...
            logger.info(
                "Query start date modified. Resetting checkpoint. Starting collection from %s",
                start_date_config_fixed,
            )
            ckpt_start_date_config_updated = common.config_to_ckpt_date_str(
                start_date_config
            )
            ckpt_start_date_updated = common.config_to_ckpt_date_str(
                start_date_config_fixed
            )
        ...
        elif not ckpt_start_date_config or (
            start_date_config != start_date_config_fixed
            and str(start_date_config) != str(ckpt_start_date_config)
        ):
            if ckpt_start_date_config:
                logger.info(
                    "Query start date modified. Invalid value found. Resuming collection from checkpoint"
                )
            else:
                logger.debug(
                    "Last known Query Start Date will be updated in checkpoint."
                )
            # Store user provided query start date in checkpoint. Continue from existing checkpoint
            ckpt_start_date_config_updated = common.config_to_ckpt_date_str(
                str(start_date_config)
            )
    ...
    if task_config.get("account", {}).get("auth_type") == _OAUTHFLOW:
        task = _list_event_log(
            task_config, meta_config, logger, helper.proxy, False, True
        )
        tasks = (
            task,
            _check_access_token(task_config, meta_config, logger, helper.proxy),
        ...
        tasks = (
            common.login_sfdc(meta_config["session_key"]),

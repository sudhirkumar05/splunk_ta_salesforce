...
def _format_query(task):
    ...
    task.add_preprocess_handler(
        "log", [logging.DEBUG, "Query event object SOQL={{query_string}}"], ""
    )
...
def _fix_source_and_sourcetype(task):
    ...
    task.add_postprocess_handler(
        "set_var",
        ["sfdc_object://{{object}}_{{account_name}}_{{input_name}}"],
        "source",
    )
...
def _stream_events(task):
    ...
    task.add_postprocess_handler(
        "log", [logging.DEBUG, "{{records|count}} events collected"]
    )
...
def check_sorting_order(logger, status_code, response):
    ...
    if error_code == "BIG_OBJECT_UNSUPPORTED_OPERATION" and bool(
        re.search("^Unsupported order direction on filter column.*ASCENDING.*", message)
    ):
...
def _add_oauth_refresh_token_tasks(
    task, task_config, meta_config, logger, helper_proxy
):
...
def validate_sort_order(task_config, meta_config, logger, helper_proxy=None):
    ...
    if task_config.get("account").get("auth_type") == _OAUTHFLOW:
        _add_oauth_refresh_token_tasks(
            task, task_config, meta_config, logger, helper_proxy
        )

    task.add_postprocess_handler(
        "check_sorting_order",
        [logger, "{{__response__.status_code}}", "{{__response__.body}}"],
        "sorting_order",
    )
...
def list_objects(task_config, meta_config, logger, helper_proxy=None):
    ...
    task.add_preprocess_handler("set_var", [False], "is_token_refreshed")
    task.add_preprocess_handler(
        "exit_if_true", ["{{finished and finished.lower() == 'true'}}"]
    )
    ...
    task.add_postprocess_handler(
        "json_path",
        ["{{records}}", '[{{(-1) if  sorting_order== "ASC" else 0}}].{{order_by}}'],
        "new_start_date",
    )
    ...
    task.add_postprocess_handler(
        ...
        ["{{is_greater_than}}", "{{records}}", "{{order_by}}", "{{new_start_date}}"],
        "records",
    )
    ...
    task.add_postprocess_handler(
        "set_var", ["{{new_start_date or start_date}}"], "start_date"
    )
...
def _check_and_set_parameters(task_conf, logger):
    for key in ("order_by", "object_fields", "object"):
        if not task_conf.get(key):
            logger.warning(
                'Field "%s" is missing for object input. Add-on is going to exit.', key
            )
            return False
    ...
    if not _check_and_set_parameters(task_config, logger):
        rest.simpleRequest(
            ...
            postargs={
                ...
                "name": "SFDC error message",
                "value": (
                    f"Some input configurations are incomplete for input \"{task_config.get('name')}\" in "
                    "Splunk Add-on for Salesforce. "
                    "Go to the Salesforce Add-on [[/app/Splunk_TA_salesforce/inputs|Inputs page]] to complete them "
                    "and to begin or to resume data collection."
                ),
            },
            method="POST",
        )
